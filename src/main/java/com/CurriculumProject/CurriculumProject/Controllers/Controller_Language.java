...
import com.CurriculumProject.CurriculumProject.Repository.Repository_Language;
...
public class Controller_Language {
	...
	@Autowired
	Repository_Language repository;
	
	@Autowired
	Repository_PersonalData repositoryPersonalData;
	
    @GetMapping("/list")
    public List<Model_Language> getAllEducation() {
    	return repository.findAll();
    }
    
    @PostMapping(path="/create/{personalDataId}")
    public Model_Language create(@PathVariable (value = "personalDataId") Integer personalDataId,
               @Valid @RequestBody Model_Language language) {
  	
        return repositoryPersonalData.findById(personalDataId).map(pdata -> {
        	language.setPersonalDataId(pdata);
	...
	public ResponseEntity<?> delete(@PathVariable Integer id) {
		...
		.orElseThrow(() -> 
		new ResourceNotFoundException("languageId " + id + " not found"));
	}
	
	@DeleteMapping(path="/delete")
	public @ResponseBody Model_Language delete (@RequestBody Model_Language n) {
		repository.delete(n);
		return n;
	}
}
